(function(){
	...
	var Chat = function( url , name ){
		...
		var post = function(type,data,success,complete){
			if( $.isFunction(data) ){complete = success ;success = data ; data={}; }
			return $.ajax({
				type: 'GET',
				data:data,
				url: url + type + "?callback=?",
				success: success,
				dataType:'json',
				complete:complete
			});
		...
		var getListener = function(tocken){
			if( !(tocken || "").length )throw("no tocken");
			var f = function(repeat){
				post("listen" , {tocken:tocken,once:!repeat?"once":""} ,
					function(list){
						$.each(list || [] ,function(i,data){
							...
							$li.attr("data-who",data.tocken == tocken ? "me" : "other");
							$list.append($li);
						});
						$list.animate({scrollTop:$list.prop("scrollHeight")});
					},
					function(){ repeat && f(repeat);}
				);
			};
			return f;
		};
		...
		post("getTocken",function(data){
			var listen = window.l = getListener(data.tocken);
			setTimeout(function(){
			...
			$form.submit(function(){
				$form.check() && post('send',$.extend(data,{"text":$form.find("textarea").val()}),function(data){
